...
if (isset($_REQUEST['id'])) {
    $makeDetails = $cars->getMakeById($_REQUEST['id']);
}
?>

<main>
    <div class="container-fluid px-4">
        <h1 class="mt-4"><?php echo $makeDetails ? 'Edit Make': 'Add Make';?></h1>
        <form id="make-form" class="make-form">
            <div class="row mb-5">
                <div class="col-md-3">
                    <div class="form-floating">
                        <input class="form-control" id="make" name="make" value="<?php echo isset($makeDetails['make']) ? $makeDetails['make'] : ''; ?>" type="text" placeholder="" required />
                        <label for="male">Make Name</label>
                    </div>
                </div>
            </div>
            <h5>Image:</h5>
            <div class="row mb-5">
                ...
                <div class="col-md-3">
                    <input type="hidden" id="image" name="image" value="<?php echo $makeDetails['image']; ?>" />
                    <div id="preview">
                        <?php 
                        if (isset($_REQUEST['id']) && $makeDetails['image'] != '') 
                        {
                            $imagePath = '../uploaded-images/make-images/' . $makeDetails['image'];
                            
                            if (file_exists($imagePath)) {
                            ?>
                            <div class="preview-image-container">
                                <img src="<?php echo $SITE_URL; ?>/uploaded-images/make-images/<?php echo $makeDetails['image']; ?>" alt="<?php echo $makeDetails['make']; ?>" class="preview-image" />
                                <button class="delete-button" onclick="deleteImage(event, <?php echo $_REQUEST['id']; ?>, '<?php echo $makeDetails['image']; ?>')">Delete</button>
                            </div>
                            <?php
                            } 
                        } 
...
<script>
    ...
    function uploadImage(chunkDataObj) {
        showSpinner();
        $.ajax({
            url: "<?php echo $SITE_URL; ?>/admin/ajax/upload_make_image.php",
            type: 'POST',
            data: chunkDataObj,
    ...
    function deleteImage(event, id, fileName) {
        ...
        event.preventDefault();
        $.ajax({
            url: "<?php echo $SITE_URL; ?>/admin/ajax/delete_make_image.php",
            type: 'POST',
            data: {
                fileName: fileName,
                id: id
    ...
    function createPreviewContainer(imagename) {
        ...
        if (imagename != '')
        {
            ...
            htmlContent = `<div class="preview-image-container"><img src="${site_url}/uploaded-images/make-images/${imagename}" alt="${imagename}" class="preview-image"></div>`;
    ...
    document.getElementById('make').addEventListener('keyup', handleInput);
    ...
    function onSave(event) {
        ...
        $.ajax({
            url: "<?php echo $SITE_URL; ?>/admin/ajax/addedit_make.php",
            data: {
            ...
            success: function(response) {
                if (response.success == true) {
                    ...
                    setTimeout(() => {
                        window.location.href = site_url+'/admin/make-addedit.php?id=' + response.edit_id;
                    }, 500);
                } else {
