...
try {
	...
	if (count($imgDataArray) == 2) {
		...
		if ($imageType !== null) {
			...
			if (file_put_contents($filePath, $imageData)) {
				...
				// INSERT PROFILE LOGO IMAGES WHILE UPLOADS.
				try {
					$result = $cars->insertProfilelogoImages($_POST);
					...
					error_log("Error inserting profile logo: " . $e->getMessage());
					$response['success'] = false;
	...
	error_log("Error in upload_profile_logo.php: " . $e->getMessage());
	http_response_code(500);
